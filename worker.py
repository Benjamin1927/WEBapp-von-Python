import poplib
# 将得到的lines组合成的text分析并且标记，然后可以用于后续的文本分析（该方法返回一个具有各种method的instance）
...
def print_info(msg, indent=0):
    ...
    if msg.is_multipart():
        ...
        if content_type == 'text/html' or content_type == 'text/plain':
            content = msg.get_payload(decode=True)  # 这里返回的是一个二进制的文本
...
# 将信息从二进制转为字符串的形式，但这个字符串仍然是编码过的，可能是别的编码方式GBK
